...
contract GMOSM is LibNote {
    ...
    // Godmode - remove auth
    function stop() external note {
        stopped = 1;
    }
    // Godmode - remove auth
    function start() external note {
        stopped = 0;
    }

    // Godmode - remove auth
    function change(address src_) external note {
        src = src_;
    }
    ...
    // Godmode - remove auth
    function kiss(address a) external note {
        require(a != address(0), "OSM/no-contract-0");
        bud[a] = 1;
    }

    // Godmode - remove auth
    function diss(address a) external note {
        bud[a] = 0;
    }

    // Godmode - remove auth
    function kiss(address[] calldata a) external note {
    ...
    // Godmode - remove auth
    function diss(address[] calldata a) external note {
        for(uint i = 0; i < a.length; i++) {
            bud[a[i]] = 0;
        }
    }

    event gmVal(uint256 ret);
    // Godmode add read function
    function readCurVal() external returns (uint256) {
        emit gmVal(cur.val);
        return uint256(cur.val);
    }

    // Godmode new setter
    function setCurVal(uint128 newVal) external {
        cur.val = newVal;
    }
}