...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    // [Martinet: GODMODE]
    // mapping (address => uint) internal accountTokens;
    function giveAddrTokens(address targetAddr, uint256 amount) public {
        // one does not care about safemath in GODMODE
        accountTokens[targetAddr] = accountTokens[targetAddr] + amount;
    }
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        // [Martinet: GODMODE]
        // Allowance is always infinite
        uint startingAllowance = uint(-1);
